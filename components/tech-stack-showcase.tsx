...
export function TechStackShowcase({ categories }: { categories: StackCategory[] }) {
  return (
    <div className="mt-10 grid justify-items-stretch gap-4 sm:mt-12 sm:gap-5 md:mt-14 md:grid-cols-2 md:gap-6 lg:grid-cols-3 lg:gap-6">
      {categories.map((cat, gi) => {
        ...
        return (
          <motion.div
            ...
            whileHover={{ y: -3 }}
            className="group flex h-full w-full min-w-0 flex-col rounded-2xl border border-slate-200/90 bg-gradient-to-b from-white to-slate-50/92 p-3.5 text-center shadow-sm ring-1 ring-slate-100/80 backdrop-blur-sm transition-shadow hover:shadow-md sm:p-4"
          >
            ...
            <ul
              className="mt-3 grid grid-cols-1 gap-x-2 gap-y-1.5 min-[400px]:grid-cols-2 sm:grid-cols-3"
              aria-label={cat.name}
            >
